...
async function checkIn() {
	try {
		...
		const data = {code: 0, msg: '签到成功！'}
		if (check_in.ret === 0) return {code: -1, msg: check_in.msg || '签到异常'};
		return data
	} catch (e) {
		console.error('e:', e)
	}
 ...
 */
async function getMsgStatus(code, msg) {
	const params = {
		from: v2freeData.title,
		to,
		subject: v2freeData.h1 + (code === 0 ? v2freeData.success : v2freeData.error),
		html: `
...
(async function () {
	...
	const { code, msg } = repData
	try {
		await getMsgStatus(code, msg);
